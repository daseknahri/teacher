...
function _renderCalendarWriteupSourcePayload(payload) {
  ...
  const summaryBadges = [];
  if (normalized.requestedProvider) summaryBadges.push(`Requested ${_escapeHtml(normalized.requestedProvider)}`);
    ...
    <div class="mt-2 flex flex-col gap-3">
      ...
      ${unitArtifacts.length ? `
        <div class="rounded-2xl border border-slate-200 bg-white p-3">
          <p class="text-[11px] font-semibold text-slate-500 uppercase tracking-[0.18em] mb-2">Helpful unit materials</p>
          <div class="flex flex-wrap gap-2 text-[12px] text-slate-600">
            ${unitArtifacts.map(artifact => `<span class="rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-blue-700">${_escapeHtml(String(artifact?.title || artifact?.id || 'Material'))}</span>`).join('')}
          </div>
        </div>` : ''}
    </div>`;
}
...
function _renderCalendarSessionMatchedGuidance(items, { canImport = false, importedIds = new Set(), hideImported = false, kindFilter = 'all' } = {}) {
...
function _renderCalendarSessionGuidanceKindFilters(items, activeKind = 'all') {
  ...
  if (!rows.length) return '';
  const summary = _summarizeCalendarRemainingGuidanceKinds(rows);
  if (summary.length <= 1) return '';
  return `
    <div class="mt-2 flex flex-wrap gap-2">
      <button class="btn btn-ghost btn-sm btn-calendar-session-guidance-kind-toggle ${activeKind === 'all' ? 'btn-primary' : ''}" data-guidance-kind="all">All</button>
      ${summary.map(([kind, count]) => `
        <button
          class="btn btn-ghost btn-sm btn-calendar-session-guidance-kind-toggle ${activeKind === kind ? 'btn-primary' : ''}"
...
function _headlineBlocksFromDetail(detail, selectedEvent) {
  ...
  if (checkedItemPaths.length) {
    const seen = new Set();
    const outlineRows = [];
    checkedItemPaths.forEach(itemPath => {
      itemPath.forEach((part, index) => {
        const prefix = itemPath.slice(0, index + 1);
        const key = prefix.map(value => String(value || '').trim().toLowerCase()).join(' > ');
        if (!key || seen.has(key)) return;
        seen.add(key);
        outlineRows.push({
          text: String(part || '').trim(),
          depth: index,
          isLeaf: index === itemPath.length - 1,
        });
      });
    });
    if (outlineRows.length) {
      return [{
        label: 'Started Outline',
        context: 'From the top heading down to the lesson steps touched in this session.',
        outlineRows,
      }];
    }
  }
...
function _renderCalendar(el, classId) {
  ...
  const selectedSessionNotePreview = selectedSessionNote.length > 180
    ...
    : selectedSessionNote;
  const selectedHeadlineCount = headlineBlocks.reduce(
    (sum, block) => sum + (
      Array.isArray(block?.outlineRows)
        ? block.outlineRows.length
  ...
  el.innerHTML = `
    ...
    return `
      ...
      ${selectedEvent ? `
      <div class="cal-detail-panel mt-3 mb-5.5 mx-3.5">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-2 mb-3">
          <div>
        ...
        <div class="flex flex-col gap-3.5">
          <div class="rounded-2xl border border-blue-100 bg-[linear-gradient(180deg,rgba(239,246,255,0.95),rgba(255,255,255,0.98))] p-3 sm:p-3.5 shadow-sm">
            <div class="flex flex-col gap-2.5">
              <div class="grid grid-cols-1 xl:grid-cols-[minmax(0,1.45fr)_minmax(260px,0.55fr)] gap-2.5 items-start">
                <div class="min-w-0 rounded-2xl border border-white/80 bg-white/75 px-3.5 py-3 shadow-sm">
                  ...
                  <p class="mt-1 text-[22px] font-semibold tracking-tight leading-tight text-slate-900 break-words">${_escapeHtml(selectedEvent.unit_title || 'Session')}</p>
          ...
          <div class="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
            ...
            <div class="mt-2 grid grid-cols-1 xl:grid-cols-[0.92fr_1.08fr_0.9fr] gap-2.5">
              <div class="rounded-2xl border border-slate-200 bg-slate-50/70 p-2.25">
      ...
      : _selectedSessionError
        ...
        : absentRows.length
          ? `<div class="mt-2 flex flex-col gap-1.25 max-h-[155px] overflow-auto pr-1">
                  ${absentRows.map(row => `
                  <div class="rounded-xl border border-slate-200 bg-white px-3 py-1.75 text-[12px] text-slate-700">
      ...
      ? `<div class="mt-2 flex flex-col gap-1.25 max-h-[155px] overflow-auto pr-1">
                 ...
                 <div class="rounded-xl border border-slate-200 bg-white px-3 py-1.75">
                   <div class="flex items-center justify-between gap-2">
  ...
  el.querySelectorAll('.btn-calendar-session-guidance-kind-toggle').forEach(button => {
    button.addEventListener('click', () => {
      _calendarSessionGuidanceKindFilter = String(button.dataset.guidanceKind || 'all').trim().toLowerCase() || 'all';
      _renderCalendar(el, classId);
    });
  });
