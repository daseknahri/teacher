...
// ============================================================
route('login', () => {
    if (isLoggedIn()) { navigate('class'); return; }
    renderLoginView();
});

route('class', authGuard(renderClassView));
route('workflow', authGuard(renderWorkflowView));
route('calendar', authGuard(renderCalendarView));
route('exams', authGuard(renderExamView));
route('owner', authGuard(() => {
    if (!isOwner()) { navigate('class'); return; }
    renderOwnerView();
}));
route('quick-planner', authGuard(renderQuickPlannerView));

fallback(() => {
    if (isLoggedIn()) navigate('class');
    else navigate('login');
});
